...
class NavierCauchy(Solver):
    ...
    def compute_loss(
        ...
        if use_pde:
            ...
            F = I + grad_u_mat
            J = torch.det(F)
            # Guard against non-physical or near-zero volumes to avoid NaNs
            eps = torch.finfo(J.dtype).eps
            J = torch.clamp(J, min=eps)
            B = F @ F.transpose(-1, -2)
            inv_J = 1.0 / J[..., None, None]
            log_J = torch.log(J)[..., None, None]
            sigma = mu * (B - I) * inv_J + lmbda * log_J * inv_J * I

            # Divergence of stress
            sigma_xx, sigma_xy, sigma_xz = sigma[..., 0, 0], sigma[..., 0, 1], sigma[..., 0, 2]
            sigma_yx, sigma_yy, sigma_yz = sigma[..., 1, 0], sigma[..., 1, 1], sigma[..., 1, 2]
            sigma_zx, sigma_zy, sigma_zz = sigma[..., 2, 0], sigma[..., 2, 1], sigma[..., 2, 2]
            ...
            pde_y = self.density * dv_dt2 - div_sigma_y
